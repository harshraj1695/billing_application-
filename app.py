...
from custom_generators import StickerGenerator, EscPosCmdGenerator, BillPdfGenerator
...
DATABASE_PATH = APP_DIR / "Database" / "store.db"


def resource_path(*parts):
    """Return a bundled read-only resource path in source and PyInstaller builds."""
    return RESOURCE_DIR.joinpath(*parts)


def prepare_runtime_files():
    """Seed writable config/database files beside the executable on first launch."""
    for source, destination in (
        (resource_path("config.json"), CONFIG_PATH),
        (resource_path("Database", "store.db"), DATABASE_PATH),
    ):
...
################ Admin Images #################
admin_login_img = Image.open(resource_path('assets', 'admin_login.png'))
login_img = Image.open(resource_path('assets', 'login.png'))
admin_img = Image.open(resource_path('assets', 'admin.png'))
inventory_icon_img = Image.open(resource_path('assets', 'inventory_icon.png'))
emp_icon_img = Image.open(resource_path('assets', 'emp_icon.png'))
invoice_icon_img = Image.open(resource_path('assets', 'invoice_icon.png'))
settings_icon_img = Image.open(resource_path('assets', 'settings_icon.png'))
logout_img = Image.open(resource_path('assets', 'logout.png'))
inventory_img = Image.open(resource_path('assets', 'inventory.png'))
update_product_img = Image.open(resource_path('assets', 'update_product.png'))
add_product_img = Image.open(resource_path('assets', 'add_product.png'))
generate_barcode_img = Image.open(resource_path('assets', 'generate_barcode.png'))
employee_img = Image.open(resource_path('assets', 'employee.png'))
add_employee_img = Image.open(resource_path('assets', 'add_employee.png'))
update_employee_img = Image.open(resource_path('assets', 'update_employee.png'))
invoice_img = Image.open(resource_path('assets', 'invoice.png'))
settings_img = Image.open(resource_path('assets', 'settings.png'))

################ Employee Images #################
emp_login_img = Image.open(resource_path('assets', 'employee_login.png'))
login_img = Image.open(resource_path('assets', 'login.png'))
billing_screen_img = Image.open(resource_path('assets', 'billing_screen.png'))
print_img = Image.open(resource_path('assets', 'print.png'))
save_img = Image.open(resource_path('assets', 'save.png'))

################ Main Images #################
main_img = Image.open(resource_path('assets', 'main.png'))
emp_login_icon_img = Image.open(resource_path('assets', 'emp_login_icon.png'))
admin_login_icon_img = Image.open(resource_path('assets', 'admin_login_icon.png'))

###############################################

config_data = load(open(CONFIG_PATH, "r"))
...
logging.basicConfig(filename=logs_directory / "app.log", level=logging.ERROR, format='%(asctime)s:%(levelname)s:%(message)s')
...
def bill_output_directory():
    """Create the folder used for saved bills beside the running app."""
    output_directory = APP_DIR / 'bills'
    output_directory.mkdir(exist_ok=True)
    return output_directory

def print_raw(commands, job_name):
    """Send ESC/POS commands to the default printer on Windows or Linux."""
    if win32print is not None:
        default_printer_name = win32print.GetDefaultPrinter()
        printer = win32print.OpenPrinter(default_printer_name, {"DesiredAccess": win32print.PRINTER_ALL_ACCESS})
        try:
            win32print.StartDocPrinter(printer, 1, (job_name, None, "RAW"))
    ...
    if shutil.which("lp") is None:
        raise RuntimeError("No printer command found. Install and configure CUPS (the 'lp' command).")
    result = run(
        ["lp", "-o", "raw", "-t", job_name],
        input=b"".join(commands),
        capture_output=True,
    )
...
class Inventory:
    ...
    def update_product(self, event=None):
        ...
        if len(self.tree.selection()) == 1:
            ...
            p_update.protocol("WM_DELETE_WINDOW", update_product_pg.cancel_update)
    ...
    def generate_barcode(self):
        ...
        svg_file_path = path.join(getcwd(), 'sticker_canvas.svg')
        sticker_generator = StickerGenerator(products, svg_file_path)
        try:
            sticker_generator.generate_stickers()
        except Exception as e:
            messagebox.showerror("Error", str(e), parent=inv)
            return
        
        # Open the generated stickers in the user's default browser. Printing can
        # then be completed from the browser's normal print dialog on any OS.
        webbrowser.open(Path(svg_file_path).resolve().as_uri())
...
class Invoice:
    ...
    def reprint_bill(self):
        ...
        for i in ids:
            ...
            esc_pos_cmds = []
            pdf_path = bill_output_directory() / f"{bill_no}.pdf"
            if pdf_path.is_file():
                webbrowser.open(pdf_path.resolve().as_uri())
                messagebox.showinfo("Re-Print", f"Opened {bill_no}.pdf. Use the PDF viewer's Print command.", parent=invc)
                continue

            bill_path = bill_output_directory() / f"{bill_no}.txt"
            if not bill_path.is_file():
                legacy_bill_path = Path("docs") / f"{bill_no}.txt"
                if legacy_bill_path.is_file():
                    bill_path = legacy_bill_path
                else:
                    messagebox.showerror("Error!!", f"Bill {bill_no} not found.", parent=invc)
                    continue
            try:
                with open(bill_path, 'rb') as f:
                    esc_pos_cmds.append(f.read())
            except Exception as e:
    ...
    def save_as_pdf(self):
        ...
        if len(ids) == 0:
            return
        for item in ids:
            bill_no = self.tree.item(item)["values"][0]
            pdf_path = bill_output_directory() / f"{bill_no}.pdf"
            if pdf_path.is_file():
                webbrowser.open(pdf_path.resolve().as_uri())
            else:
                messagebox.showerror("Error!!", f"PDF for bill {bill_no} not found.", parent=invc)
...
class BillWindow:
    ...
    def clear_updates_label(self):
        """Hide the status label even if its bill window was just closed."""
        try:
            if self.updates_label.winfo_exists():
    ...
    def generate_pdf_a5(self):
        messagebox.showinfo("Info", "A5 printing is not available yet.", parent=biller)
        return False
    ...
    def generate_thermal_bill(self):
        ...
        try:
            ...
            output_path = bill_output_directory() / f"{bill_no}.pdf"
            BillPdfGenerator(
                bill_no=bill_no,
                emp_id=emp_id,
            ...
            try:
                ...
                logging.exception(printer_error)
                messagebox.showwarning(
                    "PDF saved",
                    f"Bill saved as {output_path.name} in the bills folder.\n\n"
                    f"The thermal printer could not print it: {printer_error}",
                    parent=biller,
                )
            self.clear_updates_label()
            return True
            ...
            self.clear_updates_label()
            try:
                if biller.winfo_exists():
                    messagebox.showerror("Printing failed", str(e), parent=biller)
            except TclError:
                pass
            return False
    ...
    def print_bill(self):
        ...
        if self.thermal_printer.get() == 1:
            ...
            printed_successfully = self.generate_pdf_a5()
        elif self.a4_printer.get() == 1:
            printed_successfully = self.generate_pdf_a4()
        else:
            messagebox.showinfo("Info", "Please select a printer!", parent=biller)
            return False

        if printed_successfully and self.update_database():
            self.clear_bill()
            self.product_key_entry.focus()
            return True
        else:
